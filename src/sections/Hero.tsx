...
/* ── Full Demo modal — direct Bunny MP4 (FIX 1: native video) ── */
const FULL_DEMO_MP4_720P  = 'https://vz-b523719a-f10.b-cdn.net/9ddc752f-6ff6-49b2-9e84-cb7c5aa5a87c/play_720p.mp4';
const FULL_DEMO_MP4_1080P = 'https://vz-b523719a-f10.b-cdn.net/9ddc752f-6ff6-49b2-9e84-cb7c5aa5a87c/play_1080p.mp4';
...
/* ─── Laptop stage (looping muted preview — unchanged) ─── */
  ...
  return (
    ...
    >
      <div style={{
        ...
        overflow: 'hidden', borderRadius: '6px', background: '#fbfaf5', zIndex: 2,
        pointerEvents: 'none',
      }}>
        <video
          ref={videoRef}
          ...
          style={{ position: 'absolute', inset: 0, width: '100%', height: '100%',
            objectFit: 'cover', objectPosition: 'center', border: 0, display: 'block',
            pointerEvents: 'none', background: '#fbfaf5' }}
        >
      ...
      <button
        ...
        style={{
          ...
          background: 'rgba(0,0,0,.62)', backdropFilter: 'blur(10px)',
          WebkitBackdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,.18)',
          borderRadius: '999px', color: '#fff', fontSize: '12px', fontWeight: 700,
          fontFamily: 'inherit', padding: '7px 12px 7px 10px', cursor: 'pointer',
          pointerEvents: 'auto', lineHeight: 1, whiteSpace: 'nowrap',
        }}
      >
        {muted ? (
...
/* ─── Watch Full Demo modal — FIX 1: native HTML5 video ─────────────────────
 * Replaced the Bunny iframe with a native <video> element so the final frame
 * is correctly centred with objectFit:contain and the warm off-white background.
 * On close: pause + reset currentTime before unmount.
 * ──────────────────────────────────────────────────────────────────────────── */
function DemoModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div style={{
      ...
      padding: '20px', animation: 'st-fade-in 200ms ease forwards',
    }}>
      <style>{`
      ...
      <div onClick={onClose} aria-hidden="true" style={{
        ...
        background: 'rgba(10,8,20,.82)', backdropFilter: 'blur(14px)',
        WebkitBackdropFilter: 'blur(14px)',
      }} />

      {/* Panel */}
      <div role="dialog" aria-modal="true" aria-label="Full product demo" style={{
        position: 'relative', zIndex: 1,
        width: '90vw', maxWidth: '1200px', maxHeight: '90vh',
        background: 'rgba(18,14,30,.96)', border: '1px solid rgba(255,255,255,.1)',
        borderRadius: '20px', boxShadow: '0 40px 100px rgba(0,0,0,.6), 0 0 0 1px rgba(140,121,224,.15)',
        ...
        <div style={{
          ...
          <button onClick={onClose} aria-label="Close demo" style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            color: 'rgba(255,255,255,.7)', cursor: 'pointer', transition: 'background 150ms', flexShrink: 0,
          }}
        ...
        <div id="hero-col" className="relative z-10 flex flex-col items-center text-center"
          ...
          <div ref={ctaRef} className="flex flex-col sm:flex-row gap-3 items-center justify-center" style={{ marginBottom: '36px' }}>
            ...
            <button className="gost text-[15px] px-7 py-4" onClick={handleDemo} aria-label="Watch full demo"
              ...
              <Play size={15} aria-hidden /> Watch Full Demo
            </button>
          </div>

          <div style={{ width: '100%', maxWidth: '1060px', margin: '0 auto', position: 'relative' }}>
            <div ref={glowRef} className="absolute pointer-events-none" aria-hidden="true" style={{
              inset: '-50px',
              background:
