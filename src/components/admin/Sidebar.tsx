 ...
 * rendered as a disabled control with a "Coming later" badge — never a
 * Link to a route that doesn't exist (App.tsx registers no route for any
 * of them yet). Active-state styling comes from the current route, not
...
export default function Sidebar({ currentPath, onNavigate }: SidebarProps) {
  return (
    <nav aria-label="Studdy CRM sections" className="flex flex-col gap-1 px-3 py-4">
        ...
        if (item.status === 'comingLater') {
          return (
            ...
            >
              ...
              <span
                className="text-[9.5px] font-black uppercase tracking-wide rounded-full px-2 py-0.5"
        ...
        return (
          <a
            ...
            onClick={(e) => {
              ...
              // already where /admin renders — a real navigation would be
              // a full reload of the same page. Keep the click a no-op
              // scroll-to-top instead, and still fire onNavigate so a
              // mobile slide-over closes.
