...
| 0018 | `customer_activity_timeline.sql` | **PENDING — not yet applied.** Written for CRM-3A (Sep 2026), branch `crm-3a`, ChatGPT review round 3. Additive only: two new indexes (`account_assignments.customer_id`; `payment_events(customer_id, occurred_at desc)`) and a new `search_customer_activity_timeline(...)` SQL function that replaces the customer-detail endpoint's old full-fetch-then-paginate-in-Node timeline logic with genuine server-side, cursor/keyset-paginated pagination over one customer's merged event history. Also corrects `search_customer_pipeline()`'s (0017) `search_path` from `public, pg_catalog` to an empty string, directly in the 0017 file (still pending, edited in place). | **DO NOT RUN** until reviewed and approved per the process below. |
